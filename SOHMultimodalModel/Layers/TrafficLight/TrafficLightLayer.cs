...
namespace SOHMultimodalModel.Layers.TrafficLight
{
    public class TrafficLightLayer : AbstractActiveLayer
    {
            ...
            UnregisterAgent unregisterAgent = null)
        {
            ...
            if (lines.Length < 2)
                throw new ArgumentOutOfRangeException(nameof(metaDataFile), "There were no files specified " +
                                                                            "in the metadata.csv for the traffic light layer");
            //check header contents
            var header = lines[0].Split(',');
            if (!header.Contains("date") || !header.Contains("file"))
                throw new ArgumentOutOfRangeException(nameof(metaDataFile), "The metadata file must contain " +
                                                                            "'date' and 'file' columns");
            ...
            if (trafficLightFile.Length < 2)
                throw new ArgumentOutOfRangeException(nameof(trafficLightFile), "Files to initialize traffic " +
                    "lights need to have a header and at least one " +
                    "line with positions for the lights");
            //check header contents
            header = trafficLightFile[0].Split(',');
            if (!header.Contains("lat") || !header.Contains("lon"))
                throw new ArgumentOutOfRangeException(nameof(trafficLightFile), "The traffic light file must" +
                    " contain 'lat' and 'lon' columns");
