...
namespace SOHTests.MultimodalModelTests.CitizenTests
{
    public class CitizenTest : IClassFixture<MediatorLayerAltonaAltstadtFixture>
    {
        ...
        {
            var homeNode = _environment.GetRandomNode();
            var citizen = new Citizen
                { Gender = GenderType.Male, Worker = true, PartTimeWorker = false, StartPosition = homeNode.Position };
            citizen.Init(_layer);
            var poiPosition =
                _layer.MediatorLayer.GetNextPoiOfType(homeNode.Position, OsmFeatureCodes.Buildings, true, 100);
            Assert.Equal(poiPosition, citizen.Home.Position);
            Assert.InRange(Distance.Haversine(poiPosition.PositionArray, citizen.Position.PositionArray), 0, 100);
        }
        ...
        {
            var citizen = new Citizen { Gender = GenderType.Male, Worker = true, PartTimeWorker = false };
            citizen.Init(_layer);
            Assert.InRange(citizen.Home.Position.DistanceInKmTo(citizen.Position), 0, 0.01);

            bool AtWork() => (citizen.Tour.Current?.TripReason.Equals(TripReason.Work) ?? false) && citizen.GoalReached;
